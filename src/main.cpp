...
void coordinateTransformCircle(vector<Vec3f> circles){
	for(int i = 0; i < circles.size(); i++){
		double x_i = (lambda*circles[i][0] - 319.5*lambda)/625.5735;
		double y_i = (lambda*circles[i][1] - 239.5*lambda)/625.5735;
		cout << "Xi: " << x_i << "\tYi: " << y_i << endl;
	}
}

void coordinateTransformLine(vector<Vec4i> lines){
}
int main(int argc, char** argv){
	...
	while(1){
		...
		coordinateTransformCircle(circles);
